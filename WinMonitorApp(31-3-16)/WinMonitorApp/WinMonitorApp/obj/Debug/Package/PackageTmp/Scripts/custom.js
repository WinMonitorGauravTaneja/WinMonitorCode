...
//Component Page Functions
            ...
            success: function(data)
            {
                ...
                if (rowCountHtmlPage <= data.length)
                {
                    for (i = 0; i < data.length; i++) {
                        newRow = '<tr><td><input type="checkBox" name="masterComponentListfromDBCheckBoxes" value="' + data[i].DBMasterComponentName + '"></td><td>' + data[i].DBMasterComponentName + '</td><tr>';
    ...
    function loadComponentsFromDB() {
        ...
        $.ajax({
            ...
            success: function (data) {
                var i;
                for (i = 0; i < data.length; i++) {
                    newRow = '<tr><td><input type="checkbox" value="' + data[i].mstrComponentId + '" name="ComponentListfromComponentPageCheckBoxes"></td><td>' + data[i].mstrComponentId + '</td><td>' + data[i].mstrComponentName +
                        '</td><td>' + data[i].mstrComponentType + '</td><td>' + data[i].mstrCompanyName + '</td><td>' + data[i].mstrIncidentName + '</td></tr>';
    ...
    function addMasterComponentToDBAndPage() {
        ...
        alert("master Component Sucessfully Added");
        $("#existingComponentTableBody tr").remove();
        $("#masterPageContentPlace").load("ComponentandStatus");
...
//function to update Status of selected Components
    function updateStatusOfSelectedComponents() {
        ...
        alert(componentCheckedToUpdateStatus);
        $.ajax({
            url: "updateStatusOfSelectedComponents",
            type: 'get',
            async: false,
            data: { pstrListOfCheckBoxes: componentCheckedToUpdateStatus, pstrStatusSelected: statusOfComponentsSelected },
            datatype: 'json',
            success: function (data) {
                alert(data);
  ...
  //JQuery to add and check new incident in the incident table and JQuery to check fields in minor and major event modal
    $("#btnaddIncident").click(function () {
        ...
        if (($("#formIncidentName").val()) && ($("#formIncidentDetails").val())) {
            ...
            var jsonIncidents = { "jsonIncidentName": $("#formIncidentName").val(), "jsonIncidentDetails": $("#formIncidentDetails").val(), "jsonComponentIdList": componentCheckedToRaiseIncident };
