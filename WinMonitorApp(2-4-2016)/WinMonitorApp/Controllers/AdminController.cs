...
namespace WinMonitorApp.Controllers
{
    public class AdminController : Controller
    {
        ...
        public void addMasterComponentToDB(string pstrMasterComponentListFromPage, string pstrCompanyId)
        {
            PerformCompanyComponentWithStatusIncident performObj = new PerformCompanyComponentWithStatusIncident();
            List<DBComponent_With_Status> MasterComponentListFromPage = new List<DBComponent_With_Status>();
            DBComponent_With_Status singleMasterComponent = new DBComponent_With_Status();
            ...
            foreach (var masterComponentFromPage in distributedMasterComponents)
            {
                ...
                singleMasterComponent.DBComponentName = componentProp[1];
                singleMasterComponent.DBCSId = performObj.getseqMasterDBCSId();
                singleMasterComponent.DBStatus = "Operational";
                singleMasterComponent.DBType = "master";
                singleMasterComponent.DBCompanyId = pstrCompanyId;
                singleMasterComponent.DBMasterComponentName = componentProp[1];
                MasterComponentListFromPage.Add(singleMasterComponent);
            }
            performObj.AddComponentToDb(MasterComponentListFromPage);
        ...
        public string jsonIncidentRetreive(jsonIncidentServer jsonIncidentServers)
        {
            ...
            string catchExceptionIncident = IncidentAddObj.mSaveAddIncidentDetails(mIncidentNameSend, mIncidentDetailsSend, mComponentIdList);
        ...
        public string updateStatusOfSelectedComponents(string pstrListOfCheckBoxes, string pstrStatusSelected)
        {
            ...
            PerformCompanyComponentWithStatusIncident performObj = new PerformCompanyComponentWithStatusIncident();
            strResult = performObj.UpdatingStatusOfComponentsSelected(mListOfCheckBoxes, pstrStatusSelected);
            return (strResult);
        }

        public string SaveDataToLogTable(string pstrCompanyId)
        {
            PerformCompanyComponentWithStatusIncident performObj = new PerformCompanyComponentWithStatusIncident();
            return (performObj.SavingDataToLogTableInDB(pstrCompanyId));
