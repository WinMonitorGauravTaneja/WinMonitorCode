...
namespace WinMonitorApp.Models
{
    public class PerformCompanyComponentWithStatusIncident
    {
        ...
        {
            WinMonitorEntityModelContext mDBContext = new WinMonitorEntityModelContext();
            return mDBContext.Database.SqlQuery<DummyHistory>("select DBComponentName as ComponentName, DBIncidentName as IncidentName, DBIncidentDetails as IncidentDetails, DBDateTime as GetDateTime from DBLogHistory where DBCompanyId='"+pstringCompanyHistoryId+"'").ToList(); 
        }
        ...
        public string UpdatingStatusOfComponentsSelected(List<string> pselectedComponentList, string statusToBeChanged)
        {
            try
            {
                ...
                if (statusToBeChanged == "Operational")
                {
                    foreach (string componentId in pselectedComponentList)
                    {
                        ...
                        componentObj.DBStatus = statusToBeChanged;
                        contextObj.SaveChanges();
                    }
        ...
        public string SavingDataToLogTableInDB(string pstrCompanyId)
        {

            WinMonitorEntityModelContext contextObj = new WinMonitorEntityModelContext();
            DBIncident mDBIncidentObj = new DBIncident();
            DBComponent_With_Status mDBComponentObj = new DBComponent_With_Status();
            DBLogHistory mDBLogHistoryObj = new DBLogHistory();
            List<DBComponent_With_Status> componentList;
            string componentId;
            componentList = contextObj.Database.SqlQuery<DBComponent_With_Status>("select * from dbo.DBComponent_With_Status where DBCompanyId='" + pstrCompanyId + "';").ToList();
            foreach (DBComponent_With_Status component in componentList)
            {
                componentId = component.DBCSId;
                mDBIncidentObj = contextObj.Database.SqlQuery<DBIncident>("select * from dbo.DBIncidents where DBCSId='" + componentId + "';").FirstOrDefault();
                mDBLogHistoryObj.DBCompanyId = pstrCompanyId;
                mDBLogHistoryObj.DBCSId = component.DBCSId;
                if (mDBIncidentObj != null)
                {
                    mDBLogHistoryObj.DBIncidentId = mDBIncidentObj.DBIncidentId;
                    mDBLogHistoryObj.DBIncidentName = mDBIncidentObj.DBIncidentName;
                    mDBLogHistoryObj.DBIncidentDetails = mDBIncidentObj.DBDescription;
                }
                else
                {
                    mDBLogHistoryObj.DBIncidentId = "IncidentDefault";
                    mDBLogHistoryObj.DBIncidentName = null;
                    mDBLogHistoryObj.DBIncidentDetails = null;
                }
                mDBLogHistoryObj.DBComponentName = component.DBComponentName;

                mDBLogHistoryObj.DBDateTime = System.DateTime.Now;

                try
                {
                    //adds new incident to the incidents table
                    contextObj.DBLogHistories.Add(mDBLogHistoryObj);
